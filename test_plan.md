...
- [x] 第四阶段提供“生成处理预览”，并列展示原图与实际输出的大小、尺寸、比例、格式、质量和处理动作。
- [x] 第五阶段重新读取实际文件，校验 SHA-256、大小、宽高、比例、每坑 3–9 张及同坑比例一致。
- [x] 自动端到端测试贯通第三阶段选择、第四阶段处理和第五阶段两组坑位；源图大小、mtime、SHA-256 不变。
- [x] Python 全量回归通过（412 passed、2 skipped）；Node 12/12、`app.js` / `ui-state.js` 语法、Skill validator 和 OpenSpec strict 校验通过。
- [x] 使用历史会话重新生成第三阶段结果，页面显示真实大小与比例；114 张候选均含完整预检，普通候选不再出现“大小未知 / 比例未知”。
- [x] 使用 25.81MiB 真实图片完成第三至第五阶段浏览器验收；输出为 4.63MiB、5350×5350、1:1，派生文件位于当前任务目录且源文件 SHA-256 不变。
