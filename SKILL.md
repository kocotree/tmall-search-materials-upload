...
在交互页面中，上述第 5 步由第四阶段提交自动触发，不再要求用户单独点击 dry-run。上传任务确认页必须展示商品、目标坑位、图片数量和批准文案，不展示重复的非阻塞提醒；阻塞项仍必须留在对应任务卡片并阻止提交。所有可授权任务首次进入时默认勾选，用户取消勾选即表示本次不上传该任务，刷新或状态更新不得重新勾选已取消项。授权人输入使用第二阶段完整度巡检中去重后的负责人花名作为可选建议，同时允许手工输入。页面只允许当前仍勾选的精确 task ID 进入授权，并只提供一次“提交并自动上传所选任务”；提交后不再显示独立生产确认页。正式上传前处理器仍须在千牛当前页面实时复查目标坑位为空，自动 dry-run 不得把历史页面状态当作发布时事实。
