...
6. 第五阶段只使用两个递进子页面：`图片裁剪和压缩 → AI生成标题和描述`。第一页顶部先显示自动坑位摘要、依据、未使用候选和人工编辑器；计划未按精确 revision 确认前隐藏裁剪区。人工可增删坑位、增删/调序图片及修改比例，一张图片只能属于一个坑位；“去掉当前坑位”只移除当前坑位草稿，必须先确认，并立即使受影响的图片处理结果、文案和 dry-run 结果失效，旧预览、旧失败状态及迟到的异步结果不得重新显示，剩余坑位仍须逐坑满足 3–9 张及唯一比例门禁。确认后才在同页展开可视化 3:4/1:1 裁剪和压缩；坑位编排与图片处理区必须始终使用同一份当前坑位清单。每张图片都必须进入锁定为当前坑位比例的标准裁剪框，不得因原图接近或已经满足目标比例而绕过裁剪输出；严格符合比例的原图默认由裁剪框覆盖整张图片。正式导出的整数宽高必须严格满足 3:4 或 1:1，同一坑位全部成品必须严格为同一种比例。页面提供独立的“裁剪预校验”：只对当前已编排进坑位的图片按实际比例、裁剪框和正式编码参数生成任务内输出，逐张复核尺寸、格式、SHA-256 与不少于 204,800 字节；该步骤是本地确定性处理，不创建 handoff。裁剪预校验执行期间，除只读商品导航外，坑位删除、图片增删与调序、比例和裁剪框修改、压缩确认、页面步骤切换及继续处理按钮都必须立即禁用并显示为灰色，成功、失败或异常结束后再按当前状态恢复。预校验未通过时“完成图片处理并进入文案生成”保持禁用；商品导航按失败坑位数显示红色状态，同一坑位多张失败图片只计一个失败坑位，坑位内具体失败图片仍须标红并展示业务原因。图片、顺序、比例、裁剪框或压缩参数变化后立即使旧预校验失效。完成按钮只复核预校验指纹和实际输出，不重复执行完整试裁。标题或描述存在空项时“进入上传任务确认”保持禁用；全部填写后按钮自动启用，点击即确认当前文案并由本地确定性流程自动执行 dry-run。无阻塞时直接进入“上传任务确认”，有阻塞时自动返回当前编辑页，并在页面顶部显示具体问题和修改要求。独立 `dry_run` 阶段只保留为内部安全门禁和历史恢复状态，不显示在新任务左侧流程中。
