...
- 共享图片目录从前端配置页读取，并以稳定 `source_id + label` 引用；实际盘符或 UNC 只保存到用户数据目录的每机 `config/runtime.json`。`source_id` 必须由图片源路径身份键确定性生成：路径先执行 Unicode NFC、大小写归一和分隔符归一；盘符路径去掉 `Y:` 等盘符，UNC 路径去掉 `\\服务器` 部分，保留其余层级，再取 SHA-256 的前 12 位并加 `source-` 前缀。来源名称不参与 ID，因而“图片源 3”“视觉部模特图”“公司模特图”只要根目录相同就得到同一 ID。旧配置中的任意旧 ID 不再保存为新任务兼容别名；配置页删除后再次添加相同路径身份键时直接得到同一新 ID，不依赖历史身份数量。映射层级不同仍会得到不同路径身份键，不得猜测或自动改写层级。不得扫描盘符或假设所有电脑都映射为 `Y:`、`Z:`。`canonical_unc` 可作为本机显示/诊断建议保存，但不得参与 `source_id` 生成、团队快照选择、素材候选匹配或执行器路径绑定；可选保存最后验证身份/时间，但不得保存凭据、目录清单或图片内容。必须至少配置 1 个名称与路径均非空且不重复的来源。
