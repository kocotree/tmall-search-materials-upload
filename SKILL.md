...
首次进入“素材匹配”阶段时沿用任务配置的图片根目录，由本地页面服务从本机共享文件夹索引生成当前商品候选。`source_types` 固定规范化为 `["image"]`，不要求用户填写。页面先执行文件夹归属审查，复用整张候选文件夹卡片的点击选择方式：不显示“采用 / 排除该文件夹”下拉框或备注输入框，鼠标点击卡片或用键盘 Enter/空格切换是否纳入本轮图片加载，并以明显的已选/未选状态保存到内部 `folder_decisions`。确定性候选默认纳入，50% 连续名称粗略候选默认不纳入，用户确认后才能纳入；全部不纳入时不得加载图片。取消纳入文件夹必须立即从同商品画廊移除其候选，并同步取消来自该文件夹的已选素材与本次授权；重新纳入未被本轮画廊准备的文件夹时必须重新加载图片，不恢复旧选择。文件夹选择与已加载图库不一致时，页面必须开放“确认文件夹并重新加载图片”；选择没有变化时不得重复扫描。过滤后必须重新计算候选数、30 张分页、页码和换批按钮。保存文件夹决定时只校验 `image_roots` 为非空路径列表；随后由页面服务启动的本地 Worker 在相同 Windows 身份下检查实时可访问性。不可访问或身份变化时写入稳定 gallery-job 错误并提供“重试加载图片”，不得创建 Codex handoff。画廊必须绑定 `session_id/stage_id/prepared_from_revision/prepared_from_input_sha256/folder_decisions_sha256/prepared_folder_keys`；陈旧或跨任务画廊不得用于最终提交。`needs_user_input`/`blocked` 的素材匹配结果必须保存为只读 `review-context.json`。页面展示缩略图、来源和匹配方式，不显示“采用”复选框；点击整张图片卡片表示当前选择意图，预裁剪通过后卡片标绿并写入 `asset_decisions` 和对应的 `license_decisions`，检测不可选时卡片标红。再次点击必须立即撤销当前意图和既有决定，不能等待后台图片处理结束；后端必须按最终文件夹决定再次过滤并规范化授权记录。新图片候选必须携带稳定 `folder_id` 和 `folder_path`；历史候选缺少 `folder_id` 时按最长规范化父路径关联，无法关联时保留并标记。候选准备时把最长边不超过 640 像素的 JPEG 预览写入当前任务 `03-asset-matching/preview-cache/`，页面不得直接传输共享盘原图；内容寻址的任务预览请求必须先按受控 `asset_id` 直接命中该缓存，不得为每张缩略图反复读取或解析完整 `review-context.json`。旧任务缺少预览或使用非规范素材 ID 时才读取当前审查上下文并按需生成。预览使用短时私有缓存，选择图片时不得重建整组图片卡片。
