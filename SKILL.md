...
- 商品负责人同步和成功上传记录默认使用团队预设的两张飞书多维表格。配置页不要求普通用户填写链接、Base Token 或表 ID，只提供“授权飞书”：点击后若本机 lark-cli 尚未绑定团队应用，工作台先用随 Plugin 发布的公开 App ID 完成非交互初始化，再打开飞书官方授权页，并以当前 Windows 桌面用户身份等待授权结果、自动检测两张默认表；不得要求普通用户执行终端命令。授权链接和 device code 只允许短暂保留在工作台进程内，不得写入运行时配置、任务目录或日志；Token、应用密钥和授权原始响应不得返回页面。授权完成后保存的只有启用状态及团队表坐标，并立即把商品 ID、负责人、货号和商品名称下载为 `%LOCALAPPDATA%\tmall-search-materials\runtime\lark\product-owner-snapshot.json` 本机快照；每次打开配置页时自动更新一次，也提供“刷新负责人数据”按钮。只有完整下载且至少解析到一个商品 ID 时才能原子替换快照；读取失败、空表或写入失败时必须保留上一次成功快照。任务生成完整度巡检和负责人筛选时只读取本机快照，不得在进入完整度阶段后再次请求飞书；成功上传记录仍按已授权表实时写入。授权组件缺失、应用权限未开放或网络暂时不可用时，页面显示业务化提示并允许重试；飞书同步继续保持软失败，不得阻断本地商品表和主上传流程。
