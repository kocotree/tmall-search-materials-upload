...
2. 文件夹索引由同一 Plugin 内的 `$maintain-team-folder-index` 管理。商品选择处理器在读取候选前必须执行共享快照同步：优先校验并使用 NAS 固定目录中的最新有效不可变快照，再把可移植的 `source_id + relative_path` 按当前电脑图片源绑定重新物化到 `folder_index_root`。NAS 不可用时允许使用最后一次校验通过的本机 `team-cache`，并在扫描摘要中明确记录 `origin=local_cache`；快照哈希、canonical source 或本机绑定不一致时必须阻断。已有有效共享快照时不得启动 `index-folders`、生成新快照或刷新 NAS。共享端和本机缓存都没有有效快照时返回 `TEAM_INDEX_NO_VALID_SNAPSHOT`，不得仅因本机缺少索引而自动遍历素材源。只有用户明确要求增量更新某个素材源时，才可按 `$maintain-team-folder-index` 契约运行 `--refresh-source SOURCE` 或 `--refresh-prefix SOURCE=RELATIVE_PATH`，确认完整后显式 `publish`；不得扩大刷新范围、修改其他机器快照或自动删除历史版本。只调整商品表、名称或货号时由同步流程在本机重新匹配，不重扫 NAS。
...
`completeness` 阶段拥有专用处理器后，`resume-session`、`wait-handoff` 和页面
...
- 同一素材源发布共享快照时必须取得短时跨机器发布租约；每次发布只创建新快照并原子更新 current pointer，不覆盖其他机器已发布的快照。普通上传任务只同步和读取有效快照，不获得刷新或发布权限。
