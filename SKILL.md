...
6. 第五阶段只使用两个递进子页面：`图片裁剪和压缩 → AI生成标题和描述`。第一页顶部先显示自动坑位摘要、依据、未使用候选和人工编辑器；计划未按精确 revision 确认前隐藏裁剪区。人工可增删坑位、增删/调序图片及修改比例，一张图片只能属于一个坑位；“去掉当前坑位”只移除当前坑位草稿，必须先确认并立即按当前计划 revision 原子更新唯一 `current-slot-plan`，刷新或重新进入页面不得恢复已删除坑位；删除确认同时构成对剩余有效坑位计划的再次确认，正常删除后应直接保持图片处理可用。若剩余草稿因并发修改而不再合规，则保留已持久化的删除结果、隐藏旧图片处理区并显示业务提示，引导重新确认，不得暴露 `confirm the current slot plan before processing` 等内部状态文本。删除同时使受影响的图片处理结果、文案和 dry-run 结果失效，旧预览、旧失败状态及迟到的异步结果不得重新显示，剩余坑位仍须逐坑满足 3–9 张及唯一比例门禁。确认后才在同页展开可视化 3:4/1:1 裁剪和压缩；坑位编排与图片处理区必须始终使用同一份当前坑位清单。每张图片都必须进入锁定为当前坑位比例的标准裁剪框，不得因原图接近或已经满足目标比例而绕过裁剪输出；严格符合比例的原图默认由裁剪框覆盖整张图片。正式导出的整数宽高必须严格满足 3:4 或 1:1，同一坑位全部成品必须严格为同一种比例。页面提供独立的“裁剪预校验”：只对当前已编排进坑位的图片按实际比例、裁剪框和正式编码参数生成任务内输出，逐张复核尺寸、格式、SHA-256 与不少于 204,800 字节；该步骤是本地确定性处理，不创建 handoff。裁剪预校验执行期间，左侧阶段导航和只读商品导航保持可用；当前图片处理区域内的坑位删除、图片增删与调序、比例和裁剪框修改、压缩确认、内部子页面切换及继续处理按钮必须立即禁用并显示为灰色，成功、失败或异常结束后再按当前状态恢复。预校验未通过时“完成图片处理并进入文案生成”保持禁用；商品导航按失败坑位数显示红色状态，同一坑位多张失败图片只计一个失败坑位，坑位内具体失败图片仍须标红并展示业务原因。图片、顺序、比例、裁剪框或压缩参数变化后立即使旧预校验失效。完成按钮只复核预校验指纹和实际输出，不重复执行完整试裁。标题或描述存在空项时“进入上传任务确认”保持禁用；全部填写后按钮自动启用，点击即确认当前文案并由本地确定性流程自动执行 dry-run。无阻塞时直接进入“上传任务确认”，有阻塞时自动返回当前编辑页，并在页面顶部显示具体问题和修改要求。独立 `dry_run` 阶段只保留为内部安全门禁和历史恢复状态，不显示在新任务左侧流程中。
7. “完成图片处理并进入文案生成”把已复核输出锁定为 `outputs_ready`，同时自动创建独立 `copy_draft` 请求；该按钮提交本身就是对本次文案种子图操作的限定授权，授权只覆盖“逐坑上传顺序第一的已验证成品图、调用千牛内置 AI、读取生成文案、退出未发布表单”，并绑定 session、计划 revision、最终输出 SHA-256、坑位和首图清单。工作台后台调度器收到有效 `copy_draft` 后直接调用唯一 `process-copy-request` 处理器，不得在聊天中再次要求用户回复“同意”、授权或确认；图片、顺序、比例、裁剪结果、输出 SHA 或请求身份变化时应由处理器拒绝旧授权，而不是扩大授权范围。此授权明确不包含“填充文案”、表单确认、正式上传或发布。
   新任务不再要求用户首次点击“生成标题与描述”。已有请求失败或中断时，页面提供“继续获取”，复用同一请求、同一版本与同一 `progress.json`，只处理尚未成功的坑位，不覆盖已生成文案；重复点击必须幂等。计划、图片顺序、裁剪输出或最终输出 SHA 已变化时不得恢复旧请求，页面应引导“重新生成新版本”。“重新生成新版本”继续创建独立版本并保留历史。调度器在请求创建或恢复时被立即唤醒，并在服务启动或恢复同一精确 session 时扫描 `pending` / 可恢复的 `processing` 请求；不依赖 Codex 运行 `wait-agent-request`、聊天回复“已提交”或保持对话存活。正式处理器必须复用项目已有 `browser/qianniu_copy.py` Playwright 流程，不得复制或另写第二套千牛文案抓取脚本。处理器按商品分组；同一商品的正常流程只搜索一次商品 ID，并复用已经筛选到的商品行和空坑位身份。每个坑位都必须打开一个新的千牛“发图文”表单，验证商品锁定后上传当前坑位种子图片、执行“AI生成文案”并读取结果；进入下一坑位前原地关闭上一未确认表单，不刷新商品列表，也不重新搜索相同商品。只有表单无法原地关闭、frame 异常失效、商品页面上下文丢失，或者实时复核目标坑位失败时，单坑恢复才允许关闭残留弹窗、重新打开列表并搜索该商品一次。若重新读取后坑位已经被填充，必须记录“坑位状态已变化”并停止无效重试；若坑位仍应为空但无法定位，则记录“未找到坑位”。每坑必须使用该坑位顺序第一的最终输出图片：对任务目录中的本地最终图片复核 SHA-256 后，直接通过千牛网页的文件输入控件执行“本地上传”；为避免千牛截断长文件名和历史重名，上传事务使用包含源 SHA 前缀和随机后缀的短唯一素材名，图片字节不得改变，并按该唯一名称精确选中。不得先搜索或复用云端素材库中的同名图片，也不得使用与当前坑位无关的任意已有图片。只有读取到当前新表单中本次生成完成的标题和描述，才能把结果绑定到当前 `slot_id`；无法证明属于当前坑位的结果必须作为可恢复错误重试。进入商品列表时不得固定只查顶层页面或盲目选择 iframe；必须在限定时间内轮询顶层与全部 frame，并且仅采用“唯一商品名称/ID搜索框 + 已加载商品表格”同时成立的页面作用域。顶部帮助搜索框（例如“如何设置电子发票”）不得作为商品搜索框。该过程不得点击“填充文案”、表单最终“确认”或任何发布按钮；种子图片上传不等于批准或正式发布，正式上传仍受“上传任务确认”页一次精确提交约束。请求仍绑定计划 revision、最终输出 SHA-256 和图片顺序；每完成一个坑位立即写入同一请求的 `progress.json`，页面轮询并回填已完成文案，后续坑位失败不得丢失已完成结果。商品搜索暂时无结果、页面加载、图片上传、坑位表单或千牛 AI 返回等单坑可恢复错误在首次失败后最多重试 3 次；同一商品连续两个坑位最终失败时，必须重建一次商品页面上下文后再继续剩余坑位。首轮完成后仅对失败坑位执行一轮补跑，已成功坑位不得重新生成；补跑仍失败时停止自动循环，保留空标题和描述供用户人工填写。页面集中提示被跳过的商品、坑位，并把常见业务原因区分为“未找到坑位、上传图片失败、AI 生成超时、标题描述读取不完整”。登录失效、CDP 不可用、页面作用域歧义、授权或本地输出身份错误等整批安全异常不得跳过，仍须停止并写入统一 Agent 诊断。响应逐坑记录标题、描述、绑定商品、触发图片、依据和风险。技术异常写入统一 Agent 诊断并由页面显示稳定业务提示；只有异常持续存在时，Codex 才读取诊断、修复后重跑同一幂等入口。千牛文案必须逐坑人工核对和编辑；标题和描述全部非空时页面自动开放“进入上传任务确认”，点击后统一确认当前文案。图片、比例、顺序、裁剪结果或输出 SHA 改变时对应文案过期，并重新禁用进入按钮。千牛页面不可用时保留可重试失败状态，允许人工填写，不得自动改用 Codex 生成商品事实。

   文案生成过程中观察到的空坑位身份仅用于证明当前未发布表单属于正确商品并隔离逐坑生成结果，不得延伸为正式上传绑定，也不得作为文案完整或进入上传确认的必要条件。

   千牛本地上传等待结束后，短唯一名称仍未命中素材卡属于单坑可恢复的图片上传错误，按既有上限自动重试；同时命中多张同名素材卡属于身份歧义，必须停止整批且不得自动重试。

   “继续获取”只处理仍有空标题或空描述的坑位。每次点击必须保留标题和描述都完整的坑位，重新尝试所有空项（包括此前跳过、处理中断和已耗尽上一轮重试的坑位）；用户已经手工填满标题和描述时，该坑位立即视为文案完整，不再为了取得或刷新千牛坑位位置而重新打开页面。文案阶段读取到的远端坑位位置只作为可选审计信息，不作为继续获取、进入上传确认或正式上传的绑定。本轮仍有空项时继续显示按钮，由用户再次触发下一轮，不得自动无限循环。

   后台请求完成、失败或跳过后，页面必须立即重新计算“继续获取”等操作按钮，不得要求用户刷新页面。用户人工修改标题或描述后，该字段成为当前文案版本的权威值；迟到的轮询结果只能补齐未人工修改的字段，不得覆盖人工内容。文案提交持久化期间应暂停应用后台轮询结果，确保用户刚填写的内容先被保存和校验。
...
- 顶部飞书账号和上传确认页必须复用同一份实时授权状态；等待飞书官方页面授权期间，工作台必须持续实时复核本机 lark-cli 状态，官方授权一旦生效即自动切换为已授权，不得只返回进程内旧等待状态。状态读取必须兼容已部署的 lark-cli：优先使用 `auth status --json --verify`，若且仅若 CLI 明确不支持 `--json` 参数，则退回仍返回 JSON 的 `auth status --verify`。瞬时检测或网络失败不得被永久缓存为未授权，前端须在短暂恢复窗口内继续轮询，后续刷新也必须重新校验；已经实时确认的授权成功不得被迟到的后台失败结果覆盖。
...
不得随机打开空坑位或进入“发图文”表单。动作开始前必须先关闭受限安全弹窗；仍存在任何可见弹窗、
抽屉或对话框时，安全跳过本次随机动作并继续分页，不得强制点击弹窗下方的坑位，也不得把动作前已有
弹窗计入允许的恢复基线。该动作不得输入或搜索商品 ID，不得填写字段、上传素材、点击确认或发布。
只有打开详情时才执行最多 30 秒的完整恢复：持续核验动作前精确 URL、同一组有序商品 ID 和动作产生的
详情弹窗已关闭，三项连续稳定至少 3 次；轮询期间可重复执行既有关闭动作，不得因千牛慢加载在约 5 秒
时提前判定失败。任一随机动作执行或恢复失败都只记录诊断并跳过，不得直接中断分页采集；随后仍由正常
分页链路重新关闭安全弹窗并校验当前页。只有正常分页自身无法证明 URL、页码、商品顺序或翻页结果正确时，
才按核心分页安全门禁停止任务。没有可用商品或坑位时记录安全跳过并继续采集，不能把随机动作失败解释为
业务空值。
...
4. “素材匹配”是同一阶段内的两步流程。`folder_review` 页面展示目录元数据；文件夹列表下方、候选图片上方的“确认文件夹并加载图片”保存决定、创建 `queued` gallery job，并自动启动一次性素材执行器，不经过通用阶段提交处理器、不创建 handoff、不领取 Agent 租约，也不需要在聊天回复“已提交”。启动必须委托给现有 Explorer shell，不能直接从 UI/Codex 创建继承其令牌的子进程。执行器认领任务后，只使用当前电脑本机配置中相同 `source_id` 的根目录拼接已校验的 `relative_path`，再枚举图片、回填递归素材数、生成预览与候选；业务数据不得依赖固定盘符、RaiDrive 虚拟 UNC 绝对路径、历史 source ID 或旧绝对路径兜底。去除盘符或 UNC 服务器部分后具有相同路径身份键的本机路径可以生成同一 source ID，但执行器实际绑定必须是当前配置中的精确 `source_id`。`folder_review` 和 `gallery_preparing` 时页面底部不得再显示阶段提交按钮。选图页加载完成后只自动定位一次到第一个商品，原位置按钮为“确认选图并提交给工作台”；单图卡片点击保留原格式的预裁剪并缓存能力，提交按钮只复核缓存身份、重复项、每商品至少 3 张以及共同可行比例，然后创建 `final_material_selection` handoff。工作台后台调用 `process-final-material-handoff` 校验最终素材包并继续确定性坑位编排，不重新扫描文件夹。采用文件夹不等于选择图片，刷新页面不得自动保存或增加 revision。候选上限按商品独立计算为 100 张；枚举扩展名后必须先对全部唯一图片路径执行一次只读 `stat`，只有 200KiB–20MiB（含边界）的图片进入硬合规池，小于下限、大于上限或元数据不可读的图片不得读取、哈希、解码或生成预览。硬合规池内以 500KiB–15MiB（含边界）为优先层，200KiB–不足 500KiB 为小图补充层，大于 15MiB–20MiB 为大图补充层；每商品目标数为 `min(100, 硬合规图片总数)`。先为每个非空采用文件夹分配 1 个覆盖名额，文件夹内依次优先使用优先层、小图补充层和大图补充层；优先层数量足够时，无优先层图片的文件夹仍可使用补充层保留覆盖。其余名额先在优先层按各文件夹剩余数量比例分配，仅对不足部分依次使用小图补充层和大图补充层，不得因优先层不足而把整批退回旧式混合抽样。若单商品合规非空文件夹超过 100 个，仍保持 100 张上限，按稳定文件夹顺序确定覆盖范围并明确报告无法完整覆盖的文件夹。每个采用文件夹都保留发现数、硬合规数、三层图片数、预筛排除数、覆盖名额、比例余量、各层抽样数、总抽样数和零名额原因。系统以当前任务、商品、稳定文件夹身份和策略版本执行任务内稳定伪随机抽样；同一任务输入不变时结果不变，新任务可重新抽样。页面每批最多显示 30 张，超过 30 张时启用“换一批”，最后一批按实际余数显示。只对抽中的最多 100 张读取、校验、计算 SHA-256 和生成预览；运行中分别显示发现路径、大小预筛合规与跳过、计划检查、已检查、检查失败、内容重复、最终候选和待处理数量，完成后最终候选必须等于已检查减内容重复。结果写入当前任务的 `confirmed-gallery.json`，不得建立全量图片数据库。旧会话缺少新计数字段时只显示“历史进度口径”，不得从旧 `prepared_count` 猜测最终候选数。
   同一原图必须在一次源读取和一次解码中完成哈希、尺寸检查与任务预览；每个商品使用一个固定并发队列，所有合规候选统一占用一个工作槽，最多同时使用 8 个工作线程，不再按文件大小或格式降低并发，完成结果仍按抽样序号确定性合并。每累计 10 张有序结果立即以 `gallery_preparing` 渐进发布，页面仍按每批最多 30 张分页；后一批可以在前一批慢文件完成前进入受控调度，不得为每个发布批次重建线程池或形成读取屏障。全部候选完成前只允许浏览，不开放采用、预裁剪或最终提交。检查结果和预览只可在当前时间戳任务内通过 `gallery-checkpoint.json`、`partial-gallery.json` 按文件夹身份、素材绝对路径、源文件大小和 mtime 断点复用；同一任务调整文件夹后，继续复用仍在当前范围且源事实未变化的图片，清理已排除范围的 checkpoint，只读取新增或源事实变化的图片。素材身份或源事实变化时重算，不得建立跨任务本机缓存。checkpoint 最多累计 10 张或 1 秒批量落盘，并在每个渐进批次及任务结束时强制保存；写入使用 Windows 原子替换重试，重试后仍无法写入时只禁用本轮断点复用并继续当前图库生成，不得因可选 checkpoint 阻断素材加载。工作台进度最多每 0.75 秒持久化一次，但批次和最终状态不得丢失。目录枚举必须先按扩展名过滤并使用 `os.scandir` 单次遍历目录项元数据；同一商品采用的父子或相同物理文件夹只递归遍历其最上层覆盖根一次，再按既有文件夹排序投影递归数与图片归属；单个嵌套目录或目录项暂时不可读时记录技术计数并跳过，其他可读目录继续处理，采用根目录自身不可读时仍失败关闭。大小预筛只对每条唯一路径执行一次必要的 `stat`，不能再执行不必要的 `resolve` 或重复 `stat`。技术结果必须记录目录枚举、源文件元数据、NAS 读取、哈希、解码、预览、checkpoint 与进度持久化耗时，供稳定异常诊断使用，不得直接暴露给业务用户。
...
选图页提供一次性的“为全部商品自动选图”：按商品缺失坑位数乘以每坑最低 3 张确定目标；先选择支持 3:4 的素材，数量不足时继续补充仅支持 1:1 的素材，不得为了补充 1:1 而取消已经选中的 3:4 素材，同时支持两种比例的图片只计一次。两种比例合计仍不足时选择该商品全部可用素材。不同坑位允许使用不同的比例，但每个坑位内部仍只能使用一种比例。该过程继续复用单图预裁剪缓存、统一并发预算和跨商品重复 SHA 门禁。批量选图必须使用持续补位的预裁剪队列，单张慢文件不得形成固定批次屏障；同一轮已完成结果按小批量合并写入页面状态，过程中的自动草稿保存延后到批量结束后执行。批量选图进行中必须禁用全部选图增删操作、阶段提交、返回上一步、候选文件夹变更、重新加载图片和去掉商品，避免用户操作与自动队列互相覆盖；左侧阶段导航暂停切换但保持正常视觉，不得套用通用灰色 disabled 样式，用户尝试切换时只显示“自动选图完成后可切换”的业务提示。全部预裁剪任务完成后，恢复选图、返回和范围调整；最终提交仍只在现有逐图预检与每商品提交校验全部通过后开放。

首次进入“素材匹配”阶段时沿用任务配置的图片根目录，由本地页面服务从本机共享文件夹索引生成当前商品候选。`source_types` 固定规范化为 `["image"]`，不要求用户填写。页面先执行文件夹归属审查，复用整张候选文件夹卡片的点击选择方式：不显示“采用 / 排除该文件夹”下拉框或备注输入框，鼠标点击卡片或用键盘 Enter/空格切换是否纳入本轮图片加载，并以明显的已选/未选状态保存到内部 `folder_decisions`。确定性候选默认纳入；50% 连续名称粗略候选默认不纳入，并使用红色风险提示，用户确认属于当前商品后可手动采用。全部不纳入时不得加载图片。取消纳入文件夹必须立即从同商品画廊移除其候选，并同步取消来自该文件夹的已选素材与本次授权；重新纳入未被本轮画廊准备的文件夹时必须重新加载图片，不恢复旧选择。文件夹选择与已加载图库不一致时，页面必须开放“确认文件夹并重新加载图片”；选择没有变化时不得重复扫描。过滤后必须重新计算候选数、30 张分页、页码和换批按钮。保存文件夹决定时只校验 `image_roots` 为非空路径列表；随后由页面服务启动的本地 Worker 在相同 Windows 身份下检查实时可访问性。不可访问或身份变化时写入稳定 gallery-job 错误并提供“重试加载图片”，不得创建 Codex handoff。画廊必须绑定 `session_id/stage_id/prepared_from_revision/prepared_from_input_sha256/folder_decisions_sha256/prepared_folder_keys`；陈旧或跨任务画廊不得用于最终提交。`needs_user_input`/`blocked` 的素材匹配结果必须保存为只读 `review-context.json`。页面展示缩略图、来源和匹配方式，不显示“采用”复选框；点击整张图片卡片表示当前选择意图，预裁剪通过后卡片标绿并写入 `asset_decisions` 和对应的 `license_decisions`，检测不可选时卡片标红。再次点击必须立即撤销当前意图和既有决定，不能等待后台图片处理结束；后端必须按最终文件夹决定再次过滤并规范化授权记录。新图片候选必须携带稳定 `folder_id` 和 `folder_path`；历史候选缺少 `folder_id` 时按最长规范化父路径关联，无法关联时保留并标记。候选准备时把最长边不超过 640 像素的 JPEG 预览写入当前任务 `03-asset-matching/preview-cache/`，页面不得直接传输共享盘原图；内容寻址的任务预览请求必须先按受控 `asset_id` 直接命中该缓存，不得为每张缩略图反复读取或解析完整 `review-context.json`。旧任务缺少预览或使用非规范素材 ID 时才读取当前审查上下文并按需生成。预览使用短时私有缓存，选择图片时不得重建整组图片卡片。
...
在交互页面中，上述第 5 步由第四阶段提交自动触发，不再要求用户单独点击 dry-run。上传任务确认页必须展示商品、上传时自动选择空坑位、图片数量、批准文案，以及从当前飞书用户授权中取得的只读上传负责人，不展示重复的非阻塞提醒；阻塞项仍必须留在对应任务卡片并阻止提交。所有可授权任务首次进入时默认勾选，用户取消勾选即表示本次不上传该任务，刷新或状态更新不得重新勾选已取消项。页面不得要求用户手工填写或从商品负责人中选择上传负责人；提交时后端必须重新确认当前飞书授权有效且返回非空账号名称，并将该名称作为批准人和成功上传记录中的上传负责人。点击“提交并自动上传所选任务”时，页面先实时复核飞书授权；若本机授权已失效或缺少账号名称，应在这次用户点击中直接打开飞书官方授权页并停止本次提交，授权完成后由用户再次点击提交。若现有授权经实时复核仍有效，则同一次点击继续原有提交流程。不得回退到商品负责人、接受前端伪造姓名，或要求用户先返回任务配置。页面只允许当前仍勾选的精确 task ID 进入授权，并只提供一次“提交并自动上传所选任务”；提交后不再显示独立生产确认页。正式上传前处理器须实时读取该商品当前空坑位并为当前任务选择一个；没有空坑位时跳过该任务并继续后续任务，自动 dry-run 不得把文案阶段或历史页面中的坑位位置当作发布绑定。
7. 正式图片上传必须使用项目内 `browser/qianniu_upload.py` 中从 PlaywrightAuto 迁移的千牛搜推流程，并复用 `browser/qianniu_copy.py` 已验证的商品级页面会话与恢复基础能力。上传任务按商品首次出现顺序分组；同一商品正常情况下只进入一次列表并搜索一次商品 ID，后续任务复用当前商品行。每个任务开始时重新读取该商品当前真实空坑位，稳定选择第一个可用空坑位并打开新的“发图文”跨域表单；一次上传成功后，下一个任务必须基于更新后的页面再次读取空坑位。没有空坑位时把当前任务标记为已跳过并继续后续任务，不执行无意义重试。只有未发布表单无法原地关闭、frame 失效、商品行失效或列表上下文丢失时，才允许按需重新进入列表并搜索该商品。把批准的最终图片以 `publish-<SHA前缀>-<随机后缀>.<扩展名>` 短唯一名称上传到素材库、按该名称唯一选中、核对选择数量，再填写已批准标题和正文。正文编辑器须兼容普通 textarea/contenteditable 与 `textarea[data-cangjie-dockey]`。最终发布按钮出现前的瞬时页面或网络错误在首次失败后最多重试 3 次；每次重试前必须原地关闭当前未发布表单，重试耗尽后将当前任务标记为已跳过并继续后续任务。店铺不一致、人机验证、批准清单或任务身份异常必须停止整批，不得跳过。最终按钮优先按千牛发布专用语义 `data-autolog*=publisher_ok_clk` 唯一定位；只有明确“提交发布/发布”文字才可作为兼容入口，禁止按通用“确认/确定”文字点击。发布前保存该商品全部既有 `CopyId_value` 集合；写入 pre-publish checkpoint 后，发布按钮对该 task 最多点击一次。点击后重新读取同商品，只有旧集合完整保留且恰好新增一个 ID 才记录成功；任何点击超时或点击后异常都不得进入前述重试。千牛会把新素材前插，禁止按文案阶段或批准时的 1-based 位置推断新远端 ID。出现二次确认、点击超时、基线变化或新增 ID 不唯一时进入 `publish_uncertain` 并暂停批次。PlaywrightAuto 目录只作为迁移来源，生产运行不得依赖该外部目录。
   批次因发布按钮点击前的失败而未全部完成时，“上传任务确认”页必须展示“继续上传未完成坑位”。每次点击是对页面列出的精确未完成 task ID 的再次上传授权，并以 request ID 幂等排队；只允许把没有远端素材 ID、且仍为未开始状态或具有发布前尝试证据的 `failed/blocked` 坑位恢复到上传队列。`submitted`、`under_review`、`success` 或已有远端素材 ID 的坑位不得重复上传；存在 `uploading` 或 `publish_uncertain` 时不得开放直接继续上传，必须先完成可信远端状态确认。继续上传仍复用同一不可变批准清单、原有商品级页面会话、单坑 3 次发布前重试和逐坑飞书异步记录；本轮再次失败后保留按钮，用户可再次显式触发，禁止后台无限循环。
...
- 文件夹索引是默认入口：只保存目录元数据，不读取、哈希或统计所有图片。确定性候选默认采用，粗略候选默认排除并以红色风险提示要求确认后再采用；用户完成文件夹筛选后，才按需读取采用文件夹中的图片。
...
坑位任务：`pending_validation → needs_manual_review|ready_for_review → approved → skipped|uploading → submitted|publish_uncertain|failed → under_review|success|failed`。其中 `skipped` 只用于正式上传时实时确认商品已无空坑位的终态，不进入重试队列。

`skipped`、`publish_uncertain`、`submitted`、`under_review`、`success` 或带远端素材 ID/证据的任务不得回到自动上传队列。
