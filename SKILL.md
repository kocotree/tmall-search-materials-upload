...
1. 第三阶段候选卡片先执行原图硬性检查：文件可读，格式属于 JPG/JPEG/BMP/GIF/HEIC/PNG/WebP，文件大小在 200KiB–20MiB（含边界），且至少可按 1:1 或 3:4 最大内接裁出宽高均不少于 720px 的结果。任一硬性条件不满足时卡片显示红字并禁用“采用”。只有原图整数宽高严格满足 1:1/3:4 时才视为原生比例并继续比较对应推荐分辨率；接近目标比例但不严格相等的图片仍视为需要裁剪。未达到 3:4 的 1440×1920 或 1:1 的 1440×1440 只显示黄色提醒；可裁剪时只显示“需要裁剪”的黄色提醒，不重复判定推荐分辨率。
2. 用户点击图片卡片时立即只对这一张图片执行 1:1、3:4 最大内接预裁剪：保留原文件格式，检查裁后宽高均不少于 720px、大小仍在 200KiB–20MiB，并以素材 ID、源 SHA、图片策略和算法版本缓存结果；商品 ID 不参与图片级缓存身份，同一原图成为多个商品候选时可复用同一检查结果。正式选择决定必须继续以 `product_id + asset_id` 标识，保存和提交边界按该联合身份去重，禁止按全局 `asset_id` 覆盖商品归属。卡片点击先记录当前选择意图，只有预裁剪通过后才写入正式 `asset_decisions/license_decisions`；排队中和检查中的卡片必须始终允许再次点击取消。尚未开始的取消任务从队列移除；已开始的取消任务可继续完成任务内缓存，但迟到结果不得重新选中图片。再次选择必须复用仍在运行或已经完成的同一结果。处理普通图片最多同时运行 6 个任务，总权重预算为 8，同一素材源不单独限流；不超过 1200 万像素的普通 JPEG/WebP 权重 1，PNG/GIF/HEIC/HEIF 或 1200–3000 万像素权重 2，超过 3000 万像素权重 3，达到 6000 万像素权重 4。每个任务必须只读取、解码和 EXIF 转正原图一次，再从同一解码结果依次生成两个比例的精确编码探测；页面只更新当前素材卡片、已选列表和数量摘要，不得因单张完成而重建整批候选卡片。两个比例均失败时不保存选择并把卡片标红；选择成功后卡片标绿；只有一个比例可行时橙色提醒但允许选择。保存 1–2 张草稿合法，最终提交只汇总仍有效的单图缓存、重复 SHA 和每商品共同可行比例，不应再次逐图读取共享盘；历史草稿缺少缓存时可在首次提交中补检一次。素材匹配页允许用户通过“去掉当前商品”把单个商品从本次任务后续边界中移除；该操作必须先确认，并同步排除该商品候选文件夹、取消已选图片及本次授权，后续不再为其执行数量、比例、坑位或上传校验，但本次任务至少保留一个商品。每个仍保留的商品必须至少有 3 张预检通过、源 SHA-256 唯一且共同支持同一比例的图片。校验结果写入 `03-asset-matching/selected-asset-preflight.json`；不得遍历未采用共享盘图片，不得在第三阶段生成派生图片。
...
6. 第五阶段只使用两个递进子页面：`图片裁剪和压缩 → AI生成标题和描述`。第一页顶部先显示自动坑位摘要、依据、未使用候选和人工编辑器；计划未按精确 revision 确认前隐藏裁剪区。人工可增删坑位、增删/调序图片及修改比例，一张图片只能属于一个坑位；“去掉当前坑位”只移除当前坑位草稿，必须先确认，并立即使受影响的图片处理结果、文案和 dry-run 结果失效，剩余坑位仍须逐坑满足 3–9 张及唯一比例门禁。确认后才在同页展开可视化 3:4/1:1 裁剪和压缩。每张图片都必须进入锁定为当前坑位比例的标准裁剪框，不得因原图接近或已经满足目标比例而绕过裁剪输出；严格符合比例的原图默认由裁剪框覆盖整张图片。正式导出的整数宽高必须严格满足 3:4 或 1:1，同一坑位全部成品必须严格为同一种比例。页面提供独立的“裁剪预校验”：只对当前已编排进坑位的图片按实际比例、裁剪框和正式编码参数生成任务内输出，逐张复核尺寸、格式、SHA-256 与不少于 204,800 字节；该步骤是本地确定性处理，不创建 handoff。预校验未通过时“完成图片处理并进入文案生成”保持禁用；图片、顺序、比例、裁剪框或压缩参数变化后立即使旧预校验失效。完成按钮只复核预校验指纹和实际输出，不重复执行完整试裁。标题或描述存在空项时“进入上传任务确认”保持禁用；全部填写后按钮自动启用，点击即确认当前文案并由本地确定性流程自动执行 dry-run。无阻塞时直接进入“上传任务确认”，有阻塞时自动返回当前编辑页，并在页面顶部显示具体问题和修改要求。独立 `dry_run` 阶段只保留为内部安全门禁和历史恢复状态，不显示在新任务左侧流程中。
7. “完成图片处理并进入文案生成”把已复核输出锁定为 `outputs_ready`，同时自动创建独立 `copy_draft` 请求；该按钮提交本身就是对本次文案种子图操作的限定授权，授权只覆盖“逐坑上传顺序第一的已验证成品图、调用千牛内置 AI、读取生成文案、退出未发布表单”，并绑定 session、计划 revision、最终输出 SHA-256、坑位和首图清单。工作台后台调度器收到有效 `copy_draft` 后直接调用唯一 `process-copy-request` 处理器，不得在聊天中再次要求用户回复“同意”、授权或确认；图片、顺序、比例、裁剪结果、输出 SHA 或请求身份变化时应由处理器拒绝旧授权，而不是扩大授权范围。此授权明确不包含“填充文案”、表单确认、正式上传或发布。
   新任务不再要求用户首次点击“生成标题与描述”，只在已有请求后提供“重新生成新版本”。调度器在请求创建时被立即唤醒，并在服务启动或恢复同一精确 session 时扫描 `pending` / 可恢复的 `processing` 请求；不依赖 Codex 运行 `wait-agent-request`、聊天回复“已提交”或保持对话存活。正式处理器必须复用项目已有 `browser/qianniu_copy.py` Playwright 流程，不得复制或另写第二套千牛文案抓取脚本。处理器按商品分组；同一商品的正常流程只搜索一次商品 ID，并复用已经筛选到的商品行和空坑位身份。每个坑位都必须打开一个新的千牛“发图文”表单，验证商品锁定后上传当前坑位种子图片、执行“AI生成文案”并读取结果；进入下一坑位前原地关闭上一未确认表单，不刷新商品列表，也不重新搜索相同商品。只有表单无法原地关闭、frame 异常失效或商品页面上下文丢失时，单坑重试恢复才允许重新打开列表并搜索该商品。每坑必须使用该坑位顺序第一的最终输出图片：对任务目录中的本地最终图片复核 SHA-256 后，直接通过千牛网页的文件输入控件执行“本地上传”；为避免千牛截断长文件名和历史重名，上传事务使用包含源 SHA 前缀和随机后缀的短唯一素材名，图片字节不得改变，并按该唯一名称精确选中。不得先搜索或复用云端素材库中的同名图片，也不得使用与当前坑位无关的任意已有图片。只有读取到当前新表单中本次生成完成的标题和描述，才能把结果绑定到当前 `slot_id`；无法证明属于当前坑位的结果必须作为可恢复错误重试。进入商品列表时不得固定只查顶层页面或盲目选择 iframe；必须在限定时间内轮询顶层与全部 frame，并且仅采用“唯一商品名称/ID搜索框 + 已加载商品表格”同时成立的页面作用域。顶部帮助搜索框（例如“如何设置电子发票”）不得作为商品搜索框。该过程不得点击“填充文案”、表单最终“确认”或任何发布按钮；种子图片上传不等于批准或正式发布，正式上传仍受“上传任务确认”页一次精确提交约束。请求仍绑定计划 revision、最终输出 SHA-256 和图片顺序；每完成一个坑位立即写入同一请求的 `progress.json`，页面轮询并回填已完成文案，后续坑位失败不得丢失已完成结果。商品搜索暂时无结果、页面加载、坑位表单或千牛 AI 返回等单坑可恢复错误在首次失败后最多重试 3 次；仍失败时把该坑位记录为 `generation_status=skipped`，保留空标题和描述供用户人工填写，页面集中提示被跳过的商品、坑位及业务原因，然后继续处理后续坑位。登录失效、CDP 不可用、页面作用域歧义、授权或本地输出身份错误等整批安全异常不得跳过，仍须停止并写入统一 Agent 诊断。响应逐坑记录标题、描述、绑定商品、触发图片、依据和风险。技术异常写入统一 Agent 诊断并由页面显示稳定业务提示；只有异常持续存在时，Codex 才读取诊断、修复后重跑同一幂等入口。千牛文案必须逐坑人工核对和编辑；标题和描述全部非空时页面自动开放“进入上传任务确认”，点击后统一确认当前文案。图片、比例、顺序、裁剪结果或输出 SHA 改变时对应文案过期，并重新禁用进入按钮。千牛页面不可用时保留可重试失败状态，允许人工填写，不得自动改用 Codex 生成商品事实。
...
1. 启动前只检查运行条件：当前 Plugin 目录、用户级可写运行环境，或用于创建环境的 `uv`。不得要求只读 Plugin 安装缓存中预先存在 `.venv`。Windows 使用 `scripts/bootstrap.cmd` 在用户数据目录准备 Python 3.11、锁定依赖、虚拟环境和缓存；默认优先使用清华 TUNA 下载锁定依赖，失败后自动回退 Python 官方源，两个来源均须复用 `uv.lock` 的精确版本和 SHA-256。只有环境指纹完整且匹配当前锁文件时才可启动，下载中断产生的半成品 `.venv` 必须自动继续修复。若必须安装 `uv`，只请求安装权限；安装完成后继续启动配置页。
...
- 商品负责人同步和成功上传记录默认使用团队预设的两张飞书多维表格。配置页不要求普通用户填写链接、Base Token 或表 ID，只提供“授权飞书”：点击后若本机 lark-cli 尚未绑定团队应用，工作台先用随 Plugin 发布的公开 App ID 完成非交互初始化，再打开飞书官方授权页，并以当前 Windows 桌面用户身份等待授权结果、自动检测两张默认表；不得要求普通用户执行终端命令。授权链接和 device code 只允许短暂保留在工作台进程内，不得写入运行时配置、任务目录或日志；Token、应用密钥和授权原始响应不得返回页面。授权完成后保存的只有启用状态及团队表坐标，并立即同时下载 `%LOCALAPPDATA%\tmall-search-materials\runtime\lark\product-owner-snapshot.json` 与 `upload-history-snapshot.json` 两份本机快照；每次打开配置页时自动更新一次，也提供“刷新飞书数据”按钮。商品信息快照保存商品 ID、负责人、货号、名称和产品等级，只有完整下载且至少解析到一个商品 ID 时才能原子替换；上传历史快照只保存上传记录表“原图 SHA-256”字段中的合法指纹，空表属于有效快照。任一快照读取或写入失败时必须保留该表上一次成功快照。完整度巡检、负责人筛选和素材候选排重都只读取本机快照，不得在后续阶段逐商品或逐图片请求飞书。每个状态为 `submitted`、`under_review` 或 `success` 且已经取得远端素材 ID 的坑位必须先把成功状态持久化到本机，再立即交给单线程后台队列增量追加上传记录；飞书网络等待不得阻塞下一个千牛坑位。批次结束时按本机逐任务状态和幂等证据补齐仍未记录的成功坑位，部分批次后续恢复时也只追加新成功任务。每条记录必须包含实际发布所用原图的 SHA-256，上传记录表必须存在文本字段“原图 SHA-256”。表格读取、快照同步和上传记录追加仍保持软失败：字段不完整、权限未开放或网络暂时不可用时不得阻断本地商品表、素材处理和已经取得上传身份后的主上传流程，也不得在没有有效上传历史快照时声称已完成历史排重；但“上传任务确认”必须取得当前飞书授权账号名称作为批准人，授权组件缺失、账号未授权或姓名不可用时只阻止最终上传确认，页面提供重试引导。
...
每次完整页 checkpoint 成功后，采集器按随机 1–2 页间隔执行一次只读随机动作：只从
当前采集页实际存在的已填坑位中等概率随机选择一个具体坑位，查看后退出；不得随机打开空坑位或进入
“发图文”表单。动作开始前必须先关闭受限安全弹窗；仍存在任何可见弹窗、抽屉或对话框时，安全跳过
本次随机动作并继续分页，不得强制点击弹窗下方的坑位，也不得把动作前已有弹窗计入允许的恢复基线。
该动作不得输入或搜索商品 ID，不得填写字段、上传素材、点击确认或发布。退出后必须在最多 30 秒内持续核验动作前精确 URL、同一组有序商品 ID
和动作产生的详情弹窗已关闭，三项连续稳定至少 3 次后才能翻到下一页；轮询期间可重复执行既有关闭动作，
...
2. 文件夹索引由同一 Plugin 内的 `$maintain-team-folder-index` 管理。商品选择处理器在生成候选前必须使用配置页已校验并保存的 `team_folder_index_root` 执行共享快照同步；固定团队 UNC 仅是首次默认建议，当前电脑保存的可访问目录优先于该默认值。路径不可访问、需要 Windows 认证或没有有效来源时，把完整度阶段保持为 blocked，页面明确引导用户返回任务配置选择团队索引文件夹；保存后工作台刷新同一处理器运行时并幂等重试原 `process-product-selection`，不得要求重新提交商品选择。Plugin 不得传递、请求、记录或保存 SMB 凭据，也不得用低层命令静默连接共享。校验最新有效不可变快照后，把可移植的 `folders.csv` 缓存到 `folder_index_root/team-cache`。当前配置的路径派生 `source_id` 没有快照时，只能自动只读遍历该素材源的目录元数据、建立本机目录索引并发布第一份不可变快照，无需再次索取授权；不得按 `canonical_source`、`canonical_unc` 或路径身份键迁移、匹配、复用旧 ID 快照。已有有效共享快照时不得启动 `index-folders`、生成新快照或刷新 NAS；只有用户明确要求更新某个已有索引的素材源时，才可按 `$maintain-team-folder-index` 契约运行 `--refresh-source SOURCE` 或 `--refresh-prefix SOURCE=RELATIVE_PATH`，确认完整后显式 `publish`。共享目录暂时不可访问时允许使用同一当前 `source_id` 最后一次校验通过的本机 `team-cache`，并在任务扫描摘要中明确记录 `origin=local_cache`；快照哈希、schema 或 `source_id` 不一致时必须阻断。不得扩大刷新范围、修改其他机器快照或自动删除历史版本。商品表、名称、货号或匹配器变化时只从缓存的文件夹元数据重新匹配，不重扫 NAS。
3. 第二阶段商品选择正式提交后，工作台后台调度器只调用一次 `process-product-selection` 正式处理器。该唯一入口必须校验并领取精确 completeness handoff，使用当前任务商品快照、当前匹配器版本和已校验的 `team-cache/**/folders.csv` 即时匹配所选商品，原子生成任务级 `folder-candidates.csv`、扫描摘要和 `folder-review.json`，写回 completeness 完成结果与 asset-matching review context，并推进到素材匹配。全局 `folder_index_root/folder-candidates.csv` 即使存在也属于历史派生文件，禁止读取或作为回退。禁止再由 Codex 手工串联 `snapshot-folder-candidates`、`prepare-folder-review`、`result.json` 或阶段状态。任务目录不得包含 `folder-index.sqlite3`。任务摘要必须绑定商品快照 SHA-256、匹配器版本和来源 snapshot ID；入口必须幂等，已完成的同一 revision/SHA 只补齐或返回既有转换，不重复生成 revision。异常时必须在 `02-completeness/product-selection-diagnostic.json` 写入失败 phase、稳定 reason code、异常类型与消息、session/stage/revision/input SHA、相关文件存在性/大小/SHA、traceback 和同一入口恢复动作；能安全绑定当前 handoff 时把 completeness 写成 `blocked` 并清除 processing claim。Codex 读取该诊断、修复索引或处理器后，只能重跑同一入口，不得手工拼接半成品。该入口只读文件夹索引，不枚举或读取 NAS 图片。文件夹匹配优先使用商品 ID、完整货号和规范化商品基础名称；匹配基础名称时忽略末尾的“（主）/（副）”。除此之外，可将与基础名称具有至少 50% 最长公共连续字符、且公共连续部分不少于 5 个字符的文件夹作为粗略候选；其 `match_status` 仍保持需人工核对，但页面默认“采用”并以红色提示重点核对。若用户明确给出完整文件夹名，可用 `prepare-folder-review --exact-folder PRODUCT_ID=FOLDER_NAME` 做当前任务的一次性精确查询；不得把该查询写入别名表或自动复用于其他任务。页面必须先展示商品 ID、货号、来源、命中类型、文件夹名和完整路径；ID、货号、完整基础名称和粗略名称候选默认“采用”，用户已明确排除的决定必须保留，筛选后再读取采用文件夹中的图片；决定写入当前时间戳会话的 `folder_decisions`。
   基础名称包含 `/`、`／`、`、` 或 `|` 时，只拆成去重后的字面片段并分别检查文件夹名是否完整包含该片段，禁止拼接公共前缀、补词或推导组合名称。少于 3 个字符的片段忽略；5 个字符及以上的完整片段候选默认采用，3–4 个字符的完整短片段候选默认排除。原有 50% 粗略候选仍按未拆分基础名称计算，默认采用并以红色提示重点核对。
...
6. 搜推素材实时采集完成后，运行 `tmall-materials inspect-completeness --products <商品表> --promotion-status <promotion-material-status.csv> --output <任务目录>/02-completeness/completeness-matrix.json`。只有“搜推高价值”采集结果中的商品进入第二阶段，商品表只补充名称、货号、产品等级和负责人（“运营”列），不得扩展商品范围。把 JSON 写入当前 revision 的 `result.json.data`，页面在坑位摘要旁展示产品等级，并展示搜推素材目标/已有/缺失篇数、候选素材状态、负责人和后台证据。用户可搜索、按状态、负责人或产品等级筛选、逐项或批量选择商品；产品等级选项必须从当前巡检商品的实际等级动态生成，并允许查看未标注等级的商品。搜推素材坑位已经填满（缺失数为 0，或当前数达到目标数）的商品保留展示但必须禁用选择，批量选择和后端提交校验也不得将其加入素材匹配。提交后从 `02-completeness/input.json.values.selected_product_ids` 读取下阶段商品范围，禁止要求用户直接编辑 JSON。
...
选图页提供一次性的“为全部商品自动选图”：按商品缺失坑位数乘以每坑最低 3 张确定目标，素材不足时选择该商品全部可用素材，并继续复用单图预裁剪缓存、统一并发预算、比例一致性和跨商品重复 SHA 门禁。批量选图必须使用持续补位的预裁剪队列，单张慢文件不得形成固定批次屏障；同一轮已完成结果按小批量合并写入页面状态，过程中的自动草稿保存延后到批量结束后执行。批量选图进行中必须禁用阶段提交、返回上一步、阶段导航、候选文件夹变更、重新加载图片和去掉商品；排队中或检查中的图片仍允许再次点击取消。全部预裁剪任务完成或被取消后，恢复返回和范围调整；最终提交仍只在现有逐图预检与每商品提交校验全部通过后开放。

首次进入“素材匹配”阶段时沿用任务配置的图片根目录，由本地页面服务从本机共享文件夹索引生成当前商品候选。`source_types` 固定规范化为 `["image"]`，不要求用户填写。页面先执行文件夹归属审查，复用整张候选文件夹卡片的点击选择方式：不显示“采用 / 排除该文件夹”下拉框或备注输入框，鼠标点击卡片或用键盘 Enter/空格切换是否纳入本轮图片加载，并以明显的已选/未选状态保存到内部 `folder_decisions`。确定性候选与 50% 连续名称粗略候选均默认纳入；粗略候选的说明使用红色风险提示，用户仍可手动排除。全部不纳入时不得加载图片。取消纳入文件夹必须立即从同商品画廊移除其候选，并同步取消来自该文件夹的已选素材与本次授权；重新纳入未被本轮画廊准备的文件夹时必须重新加载图片，不恢复旧选择。文件夹选择与已加载图库不一致时，页面必须开放“确认文件夹并重新加载图片”；选择没有变化时不得重复扫描。过滤后必须重新计算候选数、30 张分页、页码和换批按钮。保存文件夹决定时只校验 `image_roots` 为非空路径列表；随后由页面服务启动的本地 Worker 在相同 Windows 身份下检查实时可访问性。不可访问或身份变化时写入稳定 gallery-job 错误并提供“重试加载图片”，不得创建 Codex handoff。画廊必须绑定 `session_id/stage_id/prepared_from_revision/prepared_from_input_sha256/folder_decisions_sha256/prepared_folder_keys`；陈旧或跨任务画廊不得用于最终提交。`needs_user_input`/`blocked` 的素材匹配结果必须保存为只读 `review-context.json`。页面展示缩略图、来源和匹配方式，不显示“采用”复选框；点击整张图片卡片表示当前选择意图，预裁剪通过后卡片标绿并写入 `asset_decisions` 和对应的 `license_decisions`，检测不可选时卡片标红。再次点击必须立即撤销当前意图和既有决定，不能等待后台图片处理结束；后端必须按最终文件夹决定再次过滤并规范化授权记录。新图片候选必须携带稳定 `folder_id` 和 `folder_path`；历史候选缺少 `folder_id` 时按最长规范化父路径关联，无法关联时保留并标记。候选准备时把最长边不超过 640 像素的 JPEG 预览写入当前任务 `03-asset-matching/preview-cache/`，页面不得直接传输共享盘原图；内容寻址的任务预览请求必须先按受控 `asset_id` 直接命中该缓存，不得为每张缩略图反复读取或解析完整 `review-context.json`。旧任务缺少预览或使用非规范素材 ID 时才读取当前审查上下文并按需生成。预览使用短时私有缓存，选择图片时不得重建整组图片卡片。

组合标题的完整字面片段属于确定性名称候选：不少于 5 字时默认采用；3–4 字时标记为短名称片段候选并默认排除。文件夹名完整包含商品基础名称时仍按名称候选处理；不得从拆分片段生成任何新的组合名称。

本地重复使用 SHA-256 排除，同一任务内同一图片不得跨商品重复选择。启用飞书同步且本机存在有效 `upload-history-snapshot.json` 时，候选检查复用原本计算的原图 SHA-256 与成功上传指纹比对；命中项不进入画廊，并从同一任务的确定性候补队列继续检查，尽量补足每商品最多 100 张候选。该比对不得额外读取或解码同一 NAS 原图，也不得在画廊加载期间实时请求飞书。只有提供后台已有图片指纹时才可声称远端去重完成；上传历史快照缺失或无效、后台仅提供素材 ID 而没有原图指纹时，最终上传前继续保持人工核对门禁。完整名称候选在文件夹归属确认前不可选择，逐文件授权未确认的候选也不可选择。
...
3. 用户在第二阶段选择商品后，以 `selected_product_ids` 作为后续商品上界；月度规则可用于后续文案或业务校验，不得重新扩展或替换该边界。第三阶段用户可用 `removed_product_ids` 显式缩小本次任务边界，但不得加入第二阶段未选择的商品。
...
在交互页面中，上述第 5 步由第四阶段提交自动触发，不再要求用户单独点击 dry-run。上传任务确认页必须展示商品、目标坑位、图片数量、批准文案，以及从当前飞书用户授权中取得的只读上传负责人，不展示重复的非阻塞提醒；阻塞项仍必须留在对应任务卡片并阻止提交。所有可授权任务首次进入时默认勾选，用户取消勾选即表示本次不上传该任务，刷新或状态更新不得重新勾选已取消项。页面不得要求用户手工填写或从商品负责人中选择上传负责人；提交时后端必须重新确认当前飞书授权有效且返回非空账号名称，并将该名称作为批准人和成功上传记录中的上传负责人。未授权或无法取得账号名称时只阻止本次上传确认，并引导用户返回任务配置检查飞书授权，不得回退到商品负责人或接受前端伪造姓名。页面只允许当前仍勾选的精确 task ID 进入授权，并只提供一次“提交并自动上传所选任务”；提交后不再显示独立生产确认页。正式上传前处理器仍须在千牛当前页面实时复查目标坑位为空，自动 dry-run 不得把历史页面状态当作发布时事实。
7. 正式图片上传必须使用项目内 `browser/qianniu_upload.py` 中从 PlaywrightAuto 迁移的千牛搜推流程，并复用 `browser/qianniu_copy.py` 已验证的商品级页面会话与恢复基础能力。上传任务按商品首次出现顺序分组；同一商品正常情况下只进入一次列表并搜索一次商品 ID，后续坑位复用当前商品行，每个坑位仍单独定位批准的 1-based 空坑位并打开新的“发图文”跨域表单。只有未发布表单无法原地关闭、frame 失效、商品行失效或列表上下文丢失时，才允许按需重新进入列表并搜索该商品。把批准的最终图片以 `publish-<SHA前缀>-<随机后缀>.<扩展名>` 短唯一名称上传到素材库、按该名称唯一选中、核对选择数量，再填写已批准标题和正文。正文编辑器须兼容普通 textarea/contenteditable 与 `textarea[data-cangjie-dockey]`。最终发布按钮出现前的瞬时页面或网络错误在首次失败后最多重试 3 次；每次重试前必须原地关闭当前未发布表单，重试耗尽后将当前任务标记为已跳过并继续后续任务。店铺不一致、人机验证、批准清单或任务身份异常必须停止整批，不得跳过。最终按钮优先按千牛发布专用语义 `data-autolog*=publisher_ok_clk` 唯一定位；只有明确“提交发布/发布”文字才可作为兼容入口，禁止按通用“确认/确定”文字点击。发布前保存该商品全部既有 `CopyId_value` 集合；写入 pre-publish checkpoint 后，发布按钮对该 task 最多点击一次。点击后重新读取同商品，只有旧集合完整保留且恰好新增一个 ID 才记录成功；任何点击超时或点击后异常都不得进入前述重试。千牛会把新素材前插，禁止按原 1-based 位置推断新远端 ID。出现二次确认、点击超时、基线变化或新增 ID 不唯一时进入 `publish_uncertain` 并暂停批次。PlaywrightAuto 目录只作为迁移来源，生产运行不得依赖该外部目录。
...
所有命令从本 Plugin 目录执行；首次使用运行 `scripts/bootstrap.cmd`。启动器默认按清华 TUNA、Python 官方源的顺序下载，并支持显式选择其他 HTTPS 镜像、用户级缓存、Python 3.11 和锁文件版本/哈希一致性检查；只有开发测试才传 `-WithTests`，只有维护者明确更新锁文件且显式指定单一来源时才传 `-UpdateLock`。
...
3. 任务配置页要求店铺确认、一个团队索引文件夹和图片源配置；不提供目标月份、商品范围或搜推采集页数，也不提供账号密码或“连接共享盘”模块。商品表、规则表和运行目录只读展示；没有可用的已保存团队索引路径时，页面只枚举 Windows 当前可见盘符并精确检查 `盘符:\浙江酷趣\天猫部\搜推素材索引-虾米`，不得递归扫描盘符。唯一命中只自动回填本机盘符路径，提交时才保存；检测和保存不得因 Windows 将映射盘解析为 UNC 而改写用户看到的盘符路径，也不得把固定 UNC 默认值写入本机配置。多处命中或未命中仍由用户选择、检测并保存。用户在“本次图片源”中新增、删除、检测并保存任意 1–50 个本机目录、映射盘或 UNC 来源；尚未保存来源或版本新增默认来源时，系统在当前可见盘符上精确检测以下五个业务目录，不递归扫描盘符：`浙江酷趣\运营中心\营销板块\小红书koc置换&买家秀\优质买家秀`、`浙江酷趣\运营中心\营销板块\小红书koc置换&淘宝买家秀\优质买家秀`、`视觉部\1-模特图`、`浙江酷趣\素材共享库\01 小红书`、`浙江酷趣\素材共享库\02 天猫买家秀`。人工素材清单与历史文件只放在高级设置。
...
页面任务状态区提供“结束当前任务”。用户确认后必须先记录可恢复的主动结束事件，再通过受管关闭事件安全停止当前工作台 Python 后端；不得删除任务目录、阶段结果或审计记录，也不得把主动结束伪装成业务流程已完成。采集 Worker、图片执行器或工作台正式处理器仍处于不可安全中断的运行状态时必须拒绝关闭并提示等待本轮处理结束，禁止直接杀进程造成半成品或不确定上传结果。
...
“上传任务确认”页只产生一次 `publish_authorization` handoff，不直接在 HTTP 请求线程中执行外部写入。该 handoff 中的精确 task ID、由后端验证的当前飞书账号批准人和 `final_confirmation=true` 同时授权工作台后台调度器调用唯一 `process-publish-authorization` 入口；入口内部顺序执行批准与发布，当前简化流程不设置授权有效期。旧 `production_confirmation` 仅供历史会话读取，不得用于新任务，也不得再次向用户索要确认。真正授权来自页面这次提交，聊天文字或调度器恢复扫描都不是授权来源；素材改变后，旧批准永远不得授权发布。可复制的启动和恢复命令见 [operations-guide.md](references/operations-guide.md)。
...
- 共享图片目录从前端配置页读取，并以稳定 `source_id + label` 引用；实际盘符或 UNC 只保存到用户数据目录的每机 `config/runtime.json`。`source_id` 必须由图片源路径身份键确定性生成：路径先执行 Unicode NFC、大小写归一，并把 Windows `\`、UNC 和 POSIX `/` 分隔符统一成 `/`；盘符路径去掉 `Y:` 等盘符，UNC 路径去掉 `\\服务器` 部分，保留其余层级，再取 SHA-256 的前 12 位并加 `source-` 前缀。来源名称不参与 ID，因而“图片源 3”“视觉部模特图”“公司模特图”只要根目录相同就得到同一 ID。旧配置中的任意旧 ID 不再保存为新任务兼容别名；配置页删除后再次添加相同路径身份键时直接得到同一新 ID，不依赖历史身份数量。映射层级不同仍会得到不同路径身份键，不得猜测或自动改写层级。不得扫描盘符或假设所有电脑都映射为 `Y:`、`Z:`。`canonical_unc` 可作为本机显示/诊断建议保存，但不得参与 `source_id` 生成、团队快照选择、素材候选匹配或执行器路径绑定；可选保存最后验证身份/时间，但不得保存凭据、目录清单或图片内容。必须至少配置 1 个名称与路径均非空且不重复的来源。
...
- 团队快照根路径可由 `TMALL_TEAM_FOLDER_INDEX_ROOT` 显式覆盖；否则使用配置页保存到本机配置的 `team_folder_index_root`。未保存时，配置页不显示或持久化固定 UNC 默认值，只对当前可见盘符拼接固定相对路径做一次有总时限的目录元数据检查；不得枚举盘内目录、触发认证、建立映射或直接写入本机配置。盘符路径的可访问性校验可以在底层解析到 UNC，但页面回填和本机配置必须继续保留检测到的盘符路径。NAS 身份使用 `team_folder_index_nas_source_id`。本机索引缓存路径优先使用 `TMALL_FOLDER_INDEX_ROOT`，其次使用 `folder_index_root`，默认使用用户数据目录下的 `cache/folder-index/`。该长期缓存只保存已校验的团队 `folders.csv` 快照及同步元数据，不保存可复用商品候选；每个时间戳任务单独即时生成并保存当前商品候选快照。
...
- 文件夹索引是默认入口：只保存目录元数据，不读取、哈希或统计所有图片。确定性候选和粗略候选默认采用；粗略候选以红色风险提示要求重点核对，用户完成文件夹筛选后，才按需读取采用文件夹中的图片。
