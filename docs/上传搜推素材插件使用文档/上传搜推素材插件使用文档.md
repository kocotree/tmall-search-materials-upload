...
```Plain Text
请帮我彻底删除“上传天猫搜推素材”的Plugin：
先结束该插件启动的工作台，再执行：
codex.cmd plugin remove tmall-search-materials@tmall-materials-team --json
codex.cmd plugin marketplace remove tmall-materials-team --json
然后只删除以下插件专属目录（不存在则跳过）：
%LOCALAPPDATA%\tmall-search-materials
%USERPROFILE%\.codex\plugins\cache\tmall-materials-team\tmall-search-materials
%USERPROFILE%\.codex\.tmp\marketplaces\tmall-materials-team
不要删除其他 Plugin、Codex 配置、浏览器数据或 NAS 文件。
```
