...
def test_macos_picker_returns_selected_absolute_directory(tmp_path):
    calls = []
    ...
    assert "choose folder" in calls[0][0][2]
    assert calls[0][1]["timeout"] == 120.0


def test_macos_picker_cancel_preserves_frontend_value():
    selected = choose_directory(
        platform="macos",
        osascript="osascript",
        runner=lambda command, **kwargs: subprocess.CompletedProcess(
            command,
            0,
            stdout="__TMALL_FOLDER_PICKER_CANCELLED__\n",
            stderr="",
        ),
    )
...
def test_macos_picker_timeout_is_reason_coded():
    def runner(command, **kwargs):
        raise subprocess.TimeoutExpired(command, kwargs["timeout"])

    with pytest.raises(FolderPickerError, match="FOLDER_PICKER_TIMEOUT"):
        choose_directory(
            platform="darwin",
            osascript="osascript",
            runner=runner,
...
def test_macos_picker_reports_gui_failure_detail():
    with pytest.raises(
        ...
        choose_directory(
            ...
            osascript="osascript",
            runner=lambda command, **kwargs: subprocess.CompletedProcess(
                command, 1, stdout="", stderr="not authorized"
            ),
        )

    assert captured.value.detail == "not authorized"


def test_macos_picker_rejects_inaccessible_result():
    with pytest.raises(
        FolderPickerError, match="FOLDER_PICKER_INVALID_RESULT"
    ):
        choose_directory(
            ...
            osascript="osascript",
            runner=lambda command, **kwargs: subprocess.CompletedProcess(
                command, 0, stdout="/missing/folder\n", stderr=""
            ),
        )
