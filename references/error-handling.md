...
| `QIANNIU_NO_EMPTY_SLOT` | 正式上传时商品已没有空坑位 | 跳过当前任务并继续后续任务，不执行无意义重试 |
...
- 同一商品的正式上传正常流程复用一次商品搜索结果；每个任务开始前从当前商品行重新读取空坑位并选择一个，没有空坑位时跳过当前任务。只有表单、frame、商品行或列表上下文失效时，重试才重新进入列表并搜索该商品。
