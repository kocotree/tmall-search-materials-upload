...
候选准备按每商品 30 张为一个优先窗口。窗口内最多同时处理 8 张轻量图片，并以总权重 12 限制 PNG、大文件等较重任务；同一素材源不另设低并发上限。每张原图只进行一次完整读取和一次解码，SHA-256、尺寸校验和 640 像素任务预览复用同一份数据；异步结果按抽样序号合并，因此完成先后不会改变候选或重复内容归属。首个窗口完成后页面立即显示可浏览的候选和当前进度，全部窗口完成前“采用”、单图预裁剪和阶段提交保持禁用。
