...
首次启动后可直接在任务配置页的“本次图片源”新增、删除和检测图片源，并点击“保存为本机配置”。配置页不再显示独立“公司共享盘”模块；用户从 Windows 原生目录窗口选择已经挂载的 Y 盘、Z 盘等目录，或直接粘贴 UNC 路径。页面支持 1–50 个“来源名称 + 根路径”。未保存图片源时，系统只在当前可见盘符上精确检测五个常用业务目录：两类 KOC/买家秀目录、`视觉部\1-模特图`、`浙江酷趣\素材共享库\01 小红书` 和 `浙江酷趣\素材共享库\02 天猫买家秀`；已有旧版三个常用来源时也会补充检测后两个新增目录，不递归扫描盘符。保存时每个来源使用稳定 `source_id + label`，实际盘符或 UNC 是当前电脑的本机绑定。`source_id` 由去掉盘符或 UNC 服务器部分后的规范化路径确定性生成，路径分隔符统一为 `/`，来源名称不参与；例如 `Y:\视觉部\1-模特图` 与 `\\192.168.124.85\视觉部\1-模特图` 得到同一 ID。旧 ID 不再作为新任务兼容别名保存或参与匹配；删除后重新选择同一路径会直接恢复同一新 ID。映射层级不同仍视为不同来源。页面默认写入用户数据目录的 `config/runtime.json`；`config/local-paths.example.json` 仅用于了解字段，不应复制回 Plugin 安装目录。配置不得保存 NAS 凭据、目录清单或图片内容。
...
先创建时间戳会话并启动任务配置页，再让用户确认店铺、团队索引文件夹和本次图片源；不配置目标月份、商品范围或搜推采集页数。缺少这些业务值不得阻止页面启动。商品表、规则表及运行目录自动发现并只读展示；团队索引文件夹选择当前电脑可访问且包含 `sources/` 的目录，图片源可配置一个或多个，两者只检测目录元数据并可保存为本机默认值。搜推素材标记为“搜推高价值”全量自动采集；本分支不准备基础素材。人工图片素材清单和历史推广素材状态位于高级设置，日常执行保持为空。

第二阶段若返回 `TEAM_INDEX_AUTHENTICATION_REQUIRED`、`TEAM_INDEX_NO_SOURCES` 或 `TEAM_INDEX_NO_VALID_LOCAL_CACHE`，页面应显示“需要选择团队索引文件夹”，而不是泛化的系统异常。用户返回任务配置选择并保存可访问目录后，受管工作台刷新处理器运行时并自动重试同一商品选择 handoff；不要重新提交配置或商品决定。Windows 负责共享目录认证，Plugin 不接收账号密码。
...
入口自动把 `folder-review.json` 写入素材匹配阶段的 `review-context.json.data`，并设置 `workflow_step=folder_review`。页面先展示目录元数据，再由独立本机请求异步回填每个文件夹的原始递归素材数；只在计数成功时显示原始递归素材数，计数中或不可访问时不显示占位技术状态，计数过程不打开、解码或哈希图片。候选文件夹不显示“采用 / 排除该文件夹”下拉框或备注输入框；整张卡片可点击，也可用 Enter/空格切换是否纳入本轮图片加载。商品 ID、完整货号和完整基础名称候选默认纳入；50% 连续名称粗略候选默认排除，使用红色提示要求确认后再采用。“确认文件夹并加载图片”必须放在文件夹列表下方、候选图片上方，是独立的本机固定操作；点击后直接调用本地 `prepare-gallery` API 保存内部决定并启动 gallery job，不经过通用阶段提交处理器。这一步不创建 handoff、不等待 Codex，也不需要回复“已提交”。图库加载完成后修改文件夹选择时，原位置按钮改为“确认文件夹并重新加载图片”；选择未变化时按钮禁用，避免重复扫描。文件夹审查和加载中隐藏页面底部提交按钮；进入选图后自动定位到第一个商品，页面底部只显示原位置的“确认选图并提交给工作台”。图片卡片整块可点击，绿色表示已选，红色表示检测后不可选，不再显示“采用”复选框。仅加载或刷新页面不得写草稿。素材匹配返回 `needs_user_input`/`blocked` 时必须保留 `review-context.json`。历史任务需要一次性精确文件夹查询时仍可单独使用 `prepare-folder-review --exact-folder`，但它不得成为新任务的正常阶段推进路径。

只有最终采用的文件夹可进入按需图片枚举，`rejected` 不得读取。候选优先来自商品 ID、完整货号或去除末尾“（主）/（副）”后的基础名称；最长公共连续部分覆盖基础名称至少 50% 且不少于 5 个字符时，可作为默认排除的粗略候选展示，并用红色提示要求确认后再采用。用户仍应核对同货号异名、主副链接和历史目录，并排除错误来源。

粗略名称候选统一默认排除，并显示红色提示；用户确认该文件夹属于当前商品后再手动采用。商品 ID、完整货号、完整基础名称及不少于 5 字的完整拆分片段候选仍默认采用，已经保存的人工决定继续优先。
...
本地 Worker 只遍历当前商品采用中的文件夹，候选上限按商品独立计算。全部唯一图片路径先经过 200KiB–20MiB 硬门槛，目标数为硬合规图片数与 100 的较小值。分配时先为每个非空采用文件夹分配 1 个覆盖名额，文件夹内依次选择 500KiB–15MiB 优先层、200KiB–不足 500KiB 小图补充层、大于 15MiB–20MiB 大图补充层；其余名额先从优先层按各文件夹剩余数量比例分配，只对缺口依次使用小图和大图补充层。优先层总量已经足够时，如果某个采用文件夹只有补充层图片，仍保留该文件夹的覆盖名额。若非空文件夹超过 100 个，候选仍限制为 100 张，按稳定文件夹顺序决定得到覆盖名额的文件夹，并返回 `FOLDER_COVERAGE_LIMIT_EXCEEDED`；空文件夹、完全重叠文件夹和未得到名额的文件夹仍保留零分配审计行。系统使用任务 ID、商品 ID、稳定文件夹 ID 和策略版本执行任务内稳定伪随机抽样；同一任务输入不变时重跑结果一致，新任务可重新抽样。只对抽中的候选读取尺寸、校验、计算 SHA-256 并生成预览，不建立全量图片数据库。同一商品内 SHA-256 相同的内容只保留一张；本机存在有效飞书成功上传快照时，还会复用同一个 SHA-256 排除历史已上传原图，并从未检查的确定性候补队列继续补足。因此“最初抽取 100 张、排除历史成功上传 3 张、继续检查 3 张候补、最终候选 100 张”是正常且可解释的结果。运行中页面显示发现路径和检查 X/Y，并分别显示失败、内容重复、历史成功上传、最终候选和待处理；完成结果必须覆盖最后一次运行消息，刷新后从完成快照恢复相同数字。旧任务没有新字段时显示“历史进度口径”，不能猜测重复数。输出中的 `candidate_strategy=proportional_task_sample`、`candidate_strategy_version=5`、`candidate_size_preference`、`sampling_identity_sha256`、`folder_allocations`、`scan_summary` 和逐商品 `requirements` 用于解释硬合规数、各层数量、各目录名额、覆盖状态和候选数。

候选准备为每个商品使用单一固定队列，最多同时处理 8 张图片；图片大小和格式不改变工作槽数量，同一素材源不另设低并发上限。每张原图只进行一次完整读取和一次解码，SHA-256、尺寸校验和 640 像素任务预览复用同一份数据；异步结果按抽样序号合并，每累计 10 张有序结果立即渐进发布，因此完成先后不会改变候选或重复内容归属。全部候选完成前“采用”、单图预裁剪和阶段提交保持禁用。

中断后点击“重试加载图片”会在同一时间戳任务内读取 `gallery-checkpoint.json`，复用 gallery identity、源大小、mtime 和预览均匹配的完成项，并继续缺失项。`partial-gallery.json`、`gallery-checkpoint.json` 与 `preview-cache/` 都只属于当前任务；新任务不会读取它们，系统也不会创建用户级跨任务图片缓存。目录发现使用 `os.scandir` 逐目录读取元数据、先按扩展名过滤，再对抽中路径执行图片内容读取，以减少大型 NAS 目录上的冗余 `resolve`/`stat`。

第三阶段候选首次显示时全部保持未选中；采用文件夹或进入候选池都不能自动选择图片。页面不显示“采用”复选框，用户点击图片卡片即确认选择意图，前端在预裁剪通过后把卡片标绿并同步写入采用与授权状态，检测不可选时卡片标红；再次点击取消。后端按最终选择项重新生成授权记录。提交时每个商品至少需要 3 张可读、策略兼容且源 SHA-256 唯一的图片；除此之外不设置低于每商品 100 张候选池的选择上限。用户选中的图片只记录稳定 `selection_order`，不提前写入坑位分组。第五阶段按 `K=min(后台缺失坑位,floor(有效唯一采用图片数/3))` 决定坑位数，并最多使用 `min(有效唯一采用图片数,K*9)` 张图片；每个图文坑位使用 3–9 张、同坑只能使用 3:4 或 1:1 且不得混合比例，超出容量的采用图片保留在未使用候选池，未在本次填充的后台空坑位继续保留为缺失。`confirmed-gallery.json` 只属于当前时间戳任务，不作为跨任务共享索引。候选准备阶段同步生成最长边不超过 640 像素的 JPEG 到 `03-asset-matching/preview-cache/`；Web 页面只传本地预览，原图保持只读并留给最终上传。旧任务首次请求时按需补建预览。Web 预览仍只允许结果中列出的文件，并额外校验原图位于已确认文件夹下，以兼容配置中的 Y/Z 映射盘与 Agent 实际解析到的 UNC 路径不同。预览响应使用短时私有缓存，卡片点击时只更新当前卡片和决定，不重新创建整个图片网格。
...
先校验商品表，再对配置页本次确认的图片来源建立只读索引；默认自动检测范围现为五个常用业务目录。只有读取失败、缺少/重复必需表头等 schema 或批次级错误、以及空表会阻断整批。缺商品 ID、非法商品 ID、重复商品 ID 是行级 blocked：`scan-summary.json` 保留对应 source row 和 reason codes，这些行不参与 ID、SKU 或名称匹配，其余有效行继续。非空但全部行为行级 blocked 时仍可完成纯 metadata 索引。以下命令使用占位商品表与隔离输出目录，仅供复制后替换；它们不表示已经扫描真实 NAS。raw indexing 不需要月份、店铺或坑位。
...
第五阶段分为三个递进子页面。第一页上方按商品分页展示第四阶段候选，下方展示唯一当前坑位草稿；默认由确定性规则生成坑位草稿，人工负责审核和调整。点击“确认坑位并进入图片裁剪”只锁定每坑 3–9 张、有序图片和唯一比例，不生成派生图。第二页按坑位显示原图尺寸、原始大小、目标比例、裁剪方式和压缩确认；用户先点击“裁剪预校验”，本地确定性处理器只对当前坑位图片生成并复核实际输出。未通过、未执行或之后修改任何图片/顺序/比例/裁剪/压缩参数时，“完成图片处理并进入文案生成”保持禁用。完成按钮只锁定已经通过的输出并自动创建 `copy_draft` 请求，同时对本次请求授予限定权限：逐坑上传首张已验证成品图、调用千牛内置 AI、读取文案并退出未发布表单。工作台后台收到该请求后直接调用固定处理器，不在聊天中再次索取“同意”或其他授权；该权限不包含填充、确认、正式上传或发布，且任一绑定图片或 SHA 变化都会使旧授权失效。固定处理器按商品复用已经筛选到的商品列表上下文，每个坑位打开一个新的“发图文”表单，完成当前坑位后原地退出；正常情况下不刷新列表或重复搜索相同商品。每坑结果单独落进度。第三页轮询工作台后台处理进度并逐坑回填千牛标题和描述；任务中断时显示“继续获取”，复用当前版本和断点，只处理未成功坑位；图片或坑位身份变化时改用“重新生成新版本”。任一标题或描述为空时“进入上传任务确认”保持禁用，全部填写后自动启用。点击后确认当前文案并进入 dry-run，不会直接触发真实上传。

单个坑位遇到商品搜索、页面加载、图片上传、表单或千牛 AI 返回等可恢复错误时，首次失败后最多自动重试 3 次；“目标坑位不可用”会先实时重读该商品空坑位，关闭残留弹窗、恢复列表并重新搜索该商品一次。若确认坑位已经被填充，则记录“坑位状态已变化”并停止无效重试；仍为空但无法定位则记录“未找到坑位”。同一商品连续两个坑位最终失败时，处理器重建一次商品页面上下文后再继续剩余坑位。首轮完成后只对失败坑位执行一轮补跑，已成功坑位不重新生成；补跑仍失败时停止自动循环，保留空标题和描述供人工填写。页面按“未找到坑位、上传图片失败、AI 生成超时、标题描述读取不完整”等业务原因列出最终跳过项。切换图片后若页面只保留无法证明属于新生成周期的旧文案，该次也按可恢复错误重试，不得回填到当前坑位。登录、浏览器连接、页面身份或本地输出身份等整批安全异常不会被跳过。

整批结束后若仍有任一标题或描述为空，页面继续显示“继续获取”。点击后保持同一文案版本，保留双字段均完整的坑位，并重新尝试所有空项，包括此前跳过、中断和已耗尽上一轮重试的坑位；双字段已经完整时不再要求绑定文案阶段看到的固定千牛坑位。每次点击仍只执行既有的有限重试和一轮失败项补跑。仍未完成时用户可以再次点击，不会自动无限循环。上一轮进度和响应保留在请求的恢复历史目录中。
...
`process-copy-request`；该入口复用 `browser/qianniu_copy.py`，按商品维持一个未确认表单、逐坑替换首图并写入 `progress.json`，不得在 HTTP 请求线程中同步运行 Playwright，也不得另写一套浏览器脚本。文案确认之前不得提交第五阶段。
# 后台恢复与旧监听兼容
