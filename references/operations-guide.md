...
整批结束后若仍有任一标题或描述为空，页面继续显示“继续获取”。点击后保持同一文案版本，保留双字段均完整的坑位，并重新尝试所有空项，包括此前跳过、中断和已耗尽上一轮重试的坑位；每次点击仍只执行既有的有限重试和一轮失败项补跑。仍未完成时用户可以再次点击，不会自动无限循环。上一轮进度和响应保留在请求的恢复历史目录中。
