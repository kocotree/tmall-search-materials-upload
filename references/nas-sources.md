...
这些共享定义不再显示在任务配置页，也不会在页面启动时按共享定义自动检测或连接。团队索引是唯一的受限发现例外：没有可用本机路径时，只对 Windows 当前可见盘符精确检查 `盘符:\浙江酷趣\天猫部\搜推素材索引-虾米`，唯一命中只回填并保存本机盘符路径；底层检测即使解析到 UNC，也不得把页面和本机配置改写为 UNC。旧版本自动写入的固定默认 UNC 按未配置处理。该过程不扫描盘内目录、不保存凭据也不连接。用户仍需先在 Windows/Explorer 中完成所需的网络访问，然后在“本次图片源”中选择已经可见的 Y 盘、Z 盘或粘贴 UNC 路径。工作台只对用户最终填写的路径进行只读元数据检测；不会打开认证窗口、读取或保存认证信息，也不会静默执行 `net use`。
