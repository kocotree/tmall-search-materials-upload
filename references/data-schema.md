...
每机图片源绑定项使用 `source_id`、`label`、`path`，可选 `canonical_unc`、`last_verified_sid`、`last_verified_at` 和 `last_status`。`source_id` 是跨电脑稳定业务引用，由 `path` 去掉盘符或 UNC 服务器部分后的 NFC、大小写与分隔符归一路径键取 SHA-256 前 12 位生成；该路径键统一使用 `/` 分隔符，`label` 不参与。`path` 是当前电脑的盘符或 UNC。旧 ID 不再保存为新任务兼容别名；已有配置中的 `image_source_history` 会被运行时忽略，并在下次保存图片源时移除。`canonical_unc` 可用于显示/诊断，但不得参与 `source_id` 生成、团队快照选择、素材候选匹配或执行器路径绑定。该配置不得进入版本库，也不得包含 NAS 凭据、目录清单或图片内容。

任务配置 handoff 的用户输入为：`store` 和 `team_folder_index_root`。店铺一致性由后台处理器在真实千牛页面自动复核，不再要求用户勾选确认；旧会话中的 `store_confirmed` 仅作兼容读取。第一阶段不接受 `month`、`product_scope`、`product_ids` 或 `promotion_max_pages`；商品范围由第二阶段的“搜推高价值”全量采集结果决定。`products_csv` 和 `rules_csv` 由项目自动发现；`team_folder_index_root` 是当前电脑可访问且包含团队 `sources/` 快照的目录，只保存到每机运行时配置；`image_source_labels` 与 `image_roots` 由可视化页面的动态图片源配置成对写入，可配置 1–50 个来源。

`GET /api/runtime/team-folder-index/discover` 只枚举 Windows 当前可见盘符，并在一个短总时限内检查固定相对路径 `浙江酷趣/天猫部/搜推素材索引-虾米`；唯一命中只回填页面中的本机盘符路径，不写本机配置。`POST /api/runtime/team-folder-index/check` 只检查目录元数据并返回可访问状态与 snapshot source 数量，即使底层校验将映射盘解析为 UNC，也必须在响应中保留用户提交的盘符路径；`PUT /api/runtime/team-folder-index` 原子保存该本机路径。旧版本自动保存的固定默认 UNC 按未配置处理，不能阻止盘符自动发现。请求携带当前 `session_id` 且该会话因 `TEAM_INDEX_*` 阻塞时，工作台更新处理器运行时并幂等重试原商品选择 handoff，不增加业务 revision，也不保存 SMB 凭据。
...
素材匹配阶段使用 `workflow_step` 区分同一阶段内的两步：目录审查为 `folder_review`，本地 Worker 生成图片期间为 `gallery_preparing`，可选图结果为 `image_selection`。文件夹确认使用 `local_action` 事务更新 revision，但不生成 `handoff.json`。`gallery-job.json` 绑定 session/stage、revision/input SHA-256、文件夹决定 SHA-256、商品边界、候选策略、图片策略和本地资源身份；每次执行保留在 `gallery-attempts/<attempt_id>/`。图片画廊还包含 `gallery_identity`：`session_id`、`stage_id`、`prepared_from_revision`、`prepared_from_input_sha256`、`folder_decisions_sha256`、`prepared_folder_keys`。点击单张图片卡片时的双比例预裁剪结果写入 `selection-preflight-cache.json`，条目以 `asset_id` 索引，并绑定源 SHA-256、图片策略 SHA-256 和算法版本，不绑定商品 ID；同一原图出现在多个商品候选中时复用检查结果。正式 `asset_decisions` 使用 `product_id + asset_id` 联合身份并在保存、提交边界去重，不得用全局 `asset_id` 覆盖商品归属。算法版本 2 表示单次读取/解码后完成双比例编码探测。前端队列另维护 `queued|running|completed|cancelled|failed`、当前选择意图、意图版本和任务权重；该瞬时状态不进入 handoff，只有预裁剪通过且仍为最新选择意图时才写入正式图片决定。该文件是可失效重建的任务内缓存，不是 handoff。最终选图提交只接受当前任务且当前采用文件夹为已准备范围子集的画廊；重新采用未准备文件夹会使画廊失效。最终校验成功后写入 `selected-asset-preflight.json`、`final-material-package.json` 和唯一的 `handoff_kind=final_material_selection` 交接。
