...
如果本次发布启用了飞书多维表格同步，在配置页的“飞书多维表格”模块中保存一次：

- 商品信息表链接和表名/表 ID：用于同步商品负责人，第二阶段即可按负责人筛选。
- 上传记录表链接和表名/表 ID：用于在素材上传成功后追加记录。

飞书授权属于当前 Codex/飞书账号的授权状态。首次检测时如果提示权限不足，需要按 Codex 的飞书授权流程补齐 Base 读取和记录创建权限；授权完成后后续任务会复用，不需要每次重新授权。未配置或暂时无权限时，Plugin 会继续使用本地商品表，不会阻断主上传流程。
