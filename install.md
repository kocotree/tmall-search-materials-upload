...
本文说明如何在另一台 Windows 11 电脑上，通过公司 GitHub 仓库的 `win` 分支手动安装“天猫搜推素材” Codex Plugin。
...
- 已安装并登录 Codex 桌面端。
- 已安装 Git for Windows，并且当前用户能够通过 HTTPS 读取项目的 GitHub 仓库；安装使用 HTTPS，不需要配置 SSH 公钥。
...
- GitHub 仓库公开时不要求登录；如果公司以后改为私有仓库，目标用户必须先获得仓库读取权限。
...
- `marketplaceSource.source` 为本项目的 GitHub HTTPS 地址。
...
先确认目标电脑能够用浏览器访问 GitHub，并检查公司网络、代理或 TLS 证书策略。公开仓库的只读安装不需要 GitHub 账号、SSH 公钥或私钥；私有仓库则需要目标用户具备读取权限。
